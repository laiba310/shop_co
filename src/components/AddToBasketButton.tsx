...
function AddToBasketButton({ product }: AddToBasketButtonProps) {
  ...
  const handleAddQuantity = () => {
    ...
    toast.success(`${product.name} added to basket!`); // Toast notification
  };
  ...
  const handleSubtractQuantity = () => {
    if (quantity > 0) {
      ...
      toast.error(`${product.name} removed from basket!`); // Toast notification
    }
  };
      ...
      <div className="flex items-center space-x-2">
        <div className="flex items-center space-x-2 px-4 py-2 rounded-full text-sm font-medium bg-gray-200 text-gray-700">
          <button
            ...
            className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors duration-200 ${
              quantity === 0 ? "bg-gray-300 cursor-not-allowed" : "hover:bg-red-600"
            }`}
          >
        ...
        <button
          ...
          className={`text-white font-medium transition-colors duration-200 px-[1rem] md:px-[10rem] py-3 rounded-full ${
            quantity === 0 ? "bg-gray-400 cursor-not-allowed" : "bg-black hover:bg-blue-700"
          }`}
        >
          Add to Cart
        </button>
