...
function BasketPage() {
  ...
  const handleRemoveItem = (id: string) => {
    useBasketStore.getState().removeItem(id);
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="flex-grow">
          {groupedItems?.map((item) => (
            <div key={item.product._id} className="relative mb-4 p-4 border rounded flex items-center justify-between">
              <button
              ...
              >
                <Trash2 fill="white" size={18} />
              </button>

              <div className="flex items-center cursor-pointer flex-1 min-w-0" onClick={() => router.push(`/product/${item.product._id}`)}>
